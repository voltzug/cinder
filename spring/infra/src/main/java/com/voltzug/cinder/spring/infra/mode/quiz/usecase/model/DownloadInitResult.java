// Cinder - zero-knowledge file transfer that burns after access
// Copyright (C) 2025  voltzug
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published
// by the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.
package com.voltzug.cinder.spring.infra.mode.quiz.usecase.model;

import com.voltzug.cinder.core.domain.valueobject.id.SessionId;
import com.voltzug.cinder.spring.infra.mode.quiz.entity.QuizQuestions;
import java.util.Objects;

/**
 * Result of initializing a download session.
 * Contains the session identifier and encrypted quiz questions for the client to decrypt and answer.
 *
 * @param sessionId the unique session identifier for this download attempt
 * @param encryptedQuestions the encrypted quiz questions (client decrypts with questionK from URL fragment)
 */
public record DownloadInitResult(
  SessionId sessionId,
  QuizQuestions encryptedQuestions
) {
  public DownloadInitResult {
    Objects.requireNonNull(sessionId, "sessionId must not be null");
    Objects.requireNonNull(encryptedQuestions, "encryptedQuestions must not be null");
  }
}