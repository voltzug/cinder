...
public class AlphaQuizUploadUseCase {
  ...
  public UploadResult upload(AlphaUploadInput input) {
    ...
    LOG.debug("Stored blob at: {}", blobPath.toString());
    ...
    LOG.info("Saved SecureFile: fileId={}, linkId={}", fileId.toString(), linkId.toString());

    _downloadLimit.initializeWithGate(fileId, linkId, specs, input.gateHash(), input.encryptedQuestions());
    LOG.debug("Initialized download limits with gate: linkId={}, retryCount={}", linkId.toString(), input.retryCount());
