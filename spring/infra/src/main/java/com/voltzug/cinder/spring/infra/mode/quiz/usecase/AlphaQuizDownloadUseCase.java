...
public class AlphaQuizDownloadUseCase {
  ...
  public DownloadInitResult initSession(LinkId linkId) {
    LOG.info("Initializing download session for linkId: {}", linkId.value());

    SecureFile<GateHash, QuizQuestions> file = _repository.findByLinkId(linkId)
      .orElseThrow(() -> {
        LOG.warn("File not found for linkId: {}", linkId.value());
        return new FileNotFoundException(null, "Link not found: " + linkId.value());
      });

    Instant now = _clock.now();
    if (file.isExpired(now)) {
      LOG.warn("File expired for linkId: {}, expiryDate: {}", linkId.value(), file.getExpiryDate());
    ...
    if (encryptedQuestions == null) {
      LOG.error("No encrypted questions found for linkId: {}", linkId.value());
  ...
  public DownloadResult verifyAndDownload(SessionId sessionId, AccessHash accessHash) {
    LOG.info("Verifying download access for sessionId: {}", sessionId.value());

    Session session = _sessionCache.get(sessionId)
      .orElseThrow(() -> {
        LOG.warn("Invalid or expired session: {}", sessionId.value());
        return new InvalidSessionException(sessionId);
      });

    LinkId linkId = session.linkId();
    if (linkId == null) {
      LOG.error("Session has no associated linkId: {}", sessionId.value());
    ...
    LOG.debug("Decremented download attempts for linkId: {}", linkId.value());

    GateHash gateHash = file.gateBox();
    if (!accessHash.canUnlock(gateHash)) {
      LOG.warn("Access verification failed for linkId: {}", linkId.value());
      throw new AccessVerificationException(linkId);
    }
    LOG.debug("Access hash verified successfully for linkId: {}", linkId.value());
    ...
    LOG.debug("Unsealed envelope and salt for linkId: {}", linkId.value());

    _sessionCache.delete(sessionId);
    LOG.debug("Deleted session after successful download: {}", sessionId.value());

    LOG.info("Download completed successfully for linkId: {}", linkId.value());
